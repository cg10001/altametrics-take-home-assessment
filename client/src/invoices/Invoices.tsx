...
export const Invoices = () => {
    ...
    return <Table className="justify-center text-slate-300">
        <TableHeader >
            <TableRow>
        ...
        <TableBody>
            {invoices.map((invoice : Invoice) =>{
                return <TableRow onClick={(() => {
                    ...
                    <TableCell>{invoice.amount != 0 ? `$ ${invoice.amount}` : ""}</TableCell>
                    <TableCell>{invoice.paid ? "Paid" : "Open"}</TableCell>
                </TableRow>
            })}
        </TableBody>
        </DialogTrigger>
            <DialogContent className="fixed font-bold top-9/20 left-1/2 p-5 text-black bg-white">
